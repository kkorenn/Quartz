...
// XPerfect.AccuracyState. It loads into the same Mono domain as Koren (UMM and
// MelonLoader share the domain), so we find its assembly by name and read those
// statics by reflection — no hard reference, so Koren is unaffected when
...
internal static class XPerfectBridge {
    ...
    // Koren mirrors XPerfect's enabled state rather than overriding it. If
