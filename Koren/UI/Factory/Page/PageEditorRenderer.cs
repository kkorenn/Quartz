...
internal static partial class PageEditor {
    ...
    public static void CreateRendererSection(Transform content) {
        ...
        GenerateUI.Input(
            ...
            "Default folder (UserData/Koren/Renders)",
            MainCore.Spr.Get(UISprite.Text128),
            "editor_render_output"
        ).Rect.AddToolTip(
            "DESC_EDITOR_RENDER_OUTPUT",
            "Folder the .mp4 files are written to. Leave blank to use UserData/Koren/Renders. Each render is named with a timestamp."
        );
        ...
        renderButton.Rect.AddToolTip(
            ...
            "Starts the offline render of whatever plays next; press again to stop and save. Begin playback after pressing — the engine runs at the chosen frame rate while recording."
        );
...
internal sealed class RecorderStatusBinder : MonoBehaviour {
    ...
    private void Update() {
        ...
        if(button.Label != null) {
            button.Label.text = recording
                ? MainCore.Tr.Get("RENDER_BUTTON_STOP", "Stop & Save")
                : MainCore.Tr.Get("RENDER_BUTTON_START", "Render");
        }
    ...
    private string BuildStatus() {
        ...
        switch(Recorder.Current) {
            case Recorder.State.Recording:
                return $"{MainCore.Tr.Get("RENDER_STATUS_RECORDING", "Recording")} — " +
                       $"{Recorder.FramesWritten} ({Recorder.OutputSeconds:0.0}s)";
